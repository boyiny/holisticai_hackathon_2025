...
def _set_env(args: argparse.Namespace) -> None:
    ...
    os.environ["CHAOS_LLM_BAD_OUTPUT_PROB"] = str(args.bad_output_prob)


def main() -> None:
    parser = argparse.ArgumentParser(description="Chaos test: LLM slow / timeout resilience")
    ...
    parser.add_argument("--jitter-min", type=int, default=2000)
    parser.add_argument("--jitter-max", type=int, default=8000)
    parser.add_argument("--net-fail-prob", type=float, default=0.1)
    parser.add_argument("--tool-fail-prob", type=float, default=0.0)
    parser.add_argument("--bad-output-prob", type=float, default=0.0)
