...
class HolisticAIBedrockChat(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        def _ensure_latin1(name: str, value: str) -> str:
            replacements = {
                "\u2013": "-",
                "\u2014": "-",
            }
            ...
            try:
                ...
                raise ValueError(
                    f"{name} contains characters outside ISO-8859-1. Please update your .env values."
                ) from exc
            return value

        headers = {
            "Content-Type": "application/json",
            "X-Team-ID": _ensure_latin1("HOLISTIC_AI_TEAM_ID", self.team_id),
            "X-API-Token": _ensure_latin1(
                "HOLISTIC_AI_API_TOKEN", self.api_token.get_secret_value()
            ),
        }
